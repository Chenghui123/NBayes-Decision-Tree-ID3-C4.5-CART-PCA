...
def face_rec():
    ...
    for k in range(1,10):
        ...
        for i in range(num_test):
            ...
            #三阶近邻
            index0 = sortedDistIndicies[0]
            index1 = sortedDistIndicies[1]
            index2 = sortedDistIndicies[2]

            if (train_label[index0] != train_label[index1]) and (train_label[index1] != train_label[index2]):
            	index3Min = index0
            elif train_label[index0] == train_label[index1]:
            	index3Min = index0
            elif train_label[index1] == train_label[index2]:
            	index3Min = index1

            if train_label[index3Min] == test_label[i]:
                true_3jie += 1
            else:
                pass
            

        accuracy1 = float(true_num)/num_test
        accuracy3 = float(true_3jie)/num_test
        x_value.append(k)
        y_value.append(round(accuracy1,2))
        z_value.append(round(accuracy3,2))
        
        print (u'当每个人选择%d张照片进行训练时，最近邻识别率为: %.2f%%'%(k,accuracy1 * 100))
    	print (u'当每个人选择%d张照片进行训练时，三阶近邻识别率为: %.2f%%'%(k,accuracy3 * 100))

    

    line1, = plt.plot(x_value,y_value,'gd-')
    line2, = plt.plot(x_value,z_value,'r^-')
    for a, b in zip(x_value, y_value):  
        plt.text(a,b,(a,b),ha='center', va='bottom', fontsize=10)  
    for a, b in zip(x_value, z_value):  
        plt.text(a,b,(a,b),ha='center', va='bottom', fontsize=10)
 
      
    plt.title(u"PCA人脸识别",fontsize=14)
    plt.xlabel(u"训练样本数",fontsize=14)
    plt.ylabel(u"准确率",fontsize=14)
    
    plt.legend(handles = [line1, line2,], labels = [u'最近邻', u'三阶近邻'], loc = 'best')
    plt.show()
